...
## 설치 및 운용
* 초기 설치 완료 후 휴대폰을 이용하여 장비 1에 접속을 한다.
* 휴대폰을 사용할 경우 "인터넷에 연결되어 있지 않음" 이번만 연결을 선택한다.
* 192.168.11.1에 접속을 한다.
  - BMS데이타의 내용을 확인한다.
    * 내부저항은 현재 읽은 값을 표시한다.   
      만일 이 값이 충전중이거나, 유효하지 않은 값일 경우 EEPROM에 있는 값을 사용하여 표시한다.   
      다만 EEPROM에 값이 없을 경우에는 유효하지 않더라도 표시 한다.
    * 기준 저항 
      EEPROM에 저장되어 있는 값이다.   
      초기 시스템 설치후 읽기를 하여 기록을 한다.   
      이 값은 임피던스 튜닝페이지에서 값을 변경할 수 있다.    
      만일 계측기를 사용하여 이 값을 보정하려면, 직접 값을 입력하거나 보정값 필드를 사용하여 입력한다. 
  - 화면의 BMS제어 로 이동한다. 
    * 초기 설치 후 기준 EEPROM에 값을 적기 위하여 "즉시 기준저항 저장 시작"을 클릭한다. 
    * 현재의 진행 상태를 중간 중간 확인한다. 
    * 만일 블루투스가 연결되어 있다면 모니터링을 할 수 있다.
  - 하단의 조회를 이용하여 값을 읽는다. 
  - 이 값이 기본 값과 같은지를 첨부된 화면과 참고 하여 비교한다.
  - 완료가 되었다면,  임피던스 튜닝으로 메뉴로 가서 현재의 읽은값과    
    기준값을 확인하여 기록한다.
  - FET를 다이오드로 보는 회로에서는    
    전류 옵셋 : 640,    
    전압 옵셋, : 16 ,   
